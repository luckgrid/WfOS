#!/usr/bin/env bash
# Archon descriptor reader: parse the canon TOML descriptor subset into JSON.
# Sourced after common.sh (uses jq). Dependency-free TOML handling via awk, mirroring
# the Dust manifest reader. Supports the regular subset the canon uses (§10.1):
...
archon_descriptor_json() {
  local f="$1" stream out scopes s nested
  stream="$(archon_descriptor_stream "$f")"
  out="$(_archon_scope_object "$stream" .)"
  scopes="$(printf '%s\n' "$stream" | awk -F'\037' '$1!="." {print $1}' | sort -u)"
  for s in $scopes; do
    nested="$(_archon_scope_object "$stream" "$s")"
...
# Colocated-first authoring; central (archon/descriptors/) is a valid override location.
# Colocated locations: each Build/src/workspaces/<ws>/ root, and wfos packages/<pkg>/ and
# apps/<app>/ directories.
archon_find_descriptors() {
  local f wsdir
  for f in "$ARCHON_DESCRIPTORS"/*.descriptor.toml; do
