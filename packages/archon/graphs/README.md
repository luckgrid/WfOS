# Archon graphs — project relationship graph
...
- **Nodes:** `{ id, kind }` where `kind` is one of `native-substrate`, `workspace`, `package`,
...
The graph is **generated**, not hand-authored. `moon run archon:sync` derives it from the
...
dust        -> governed-by -> policy:dust.agent
...
A future Kraken `krk graph` (H09) consumes the same artifact.
