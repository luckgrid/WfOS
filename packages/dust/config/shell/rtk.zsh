# WfOS Dust — RTK output-compression layer (zsh).
...
# whole layer is a no-op unless RTK is installed AND DUST_RTK=1 (default). Set DUST_RTK=0 to
...
if [ "${DUST_RTK}" = "1" ] && command -v rtk >/dev/null 2>&1 && rtk gain --help >/dev/null 2>&1; then
  ...
  dust-rtk() { rtk "$@"; }    # raw passthrough to the proxy

  export DUST_RTK_ACTIVE=1
fi
