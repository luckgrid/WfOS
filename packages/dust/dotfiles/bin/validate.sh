#!/usr/bin/env bash
# validate.sh — dry-run gate for the Dust chezmoi source (WfOS).
#
...
# or `moon run dust:validate-dotfiles`.
#
...
if [ -f "$frag" ]; then
  ...
  # dust.zsh must stand down when the chezmoi plugin layer is active (avoid double-sourcing)
  dustzsh="$SRC/../config/shell/dust.zsh"
  if [ -f "$dustzsh" ]; then
    grep -q 'DUST_PLUGINS_MANAGED' "$dustzsh" && note "dust.zsh respects DUST_PLUGINS_MANAGED" \
      || bad "dust.zsh does not stand down for the chezmoi plugin layer"
  fi
...
if command -v chezmoi >/dev/null 2>&1; then
  ...
  note "chezmoi binary NOT installed (manifest dotfiles/chezmoi module; install via dust bootstrap)"
